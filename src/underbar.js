...
(function() {
  ...
  _.map = function(collection, iterator) {
    ...
    var transArray=[];
    for(var i =0;i<collection.length;i++){
      transArray.push(iterator(collection[i],i,collection));
    }
    return transArray;
  };
  ...
  // not efficient way to solve using two for loops. Could consolidate into 1.
  _.invoke = function(collection, functionOrKey, args) {
    var invArr =[];
    if(typeof(functionOrKey)=='function'){
      for(var i =0; i<collection.length;i++){
        invArr[i]=functionOrKey.apply(collection[i]);
      }
    }
    else{
      for(var i=0; i<collection.length;i++){
        invArr[i]=collection[i][functionOrKey].apply(collection[i]);
      }
    }
    return invArr;
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
    accumulator!=undefined ? accumulator =accumulator: accumulator=collection[0];
  ...
  _.every = function(collection, iterator) {
    if(!iterator)iterator=_.identity;
      return _.reduce(collection,function(found,i){
          return found && !!iterator(i);
      },true)
  ...
  _.some = function(collection, iterator) {
    ...
    // every will eval true when all items are pass as true 
    // manipulate every to eval true when not all items are not true.
  ...
  _.memoize = function(func) {
    //use arguments as properties and return value of function as property values
    var used={};
    return function(){
      var param= Array.prototype.slice.call(arguments);
      if(param in used) return used[param];
      else return(used[param]=func.apply(this,param));
    };
  };
  ...
  _.delay = function(func, wait) {
      ...
      setTimeout(function() { func.apply(null, param); }, wait);
  };
